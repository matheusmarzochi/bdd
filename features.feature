...
#Somente usuários master podem excluir os registros
#Quando inserir uma nova jornada/ampliação/redução deve aplicar a regra de atualizar a data fim do registro anterior com a data início declara -1 dia
...
Feature: Funcionalidade de Cadastro da Jornada, Ampliação e Redução da aba Contratual dentro do cadastro do funcionário.
  ...
  Scenario: 5 Cadastrando uma redução maior que a quantidade de aulas ampliadas anteriormente.
    ...
    When eu for lançar uma "Redução" e autorizar 20
    And clicar em "Salvar"
  ...
  Scenario: 6 Inserindo uma segunda Jornada, Ampliação ou Redução para inserir data fim na Jornada anterior.
    ...
    And "Data Fim" igual a 10/12/2016
    When eu inserir uma nova "Amplicação" com data inicio igual a 22/12/16
    And sem data fim
    And clicar em "Salvar"
    Then o sistema deve atualizar a "Data Fim" do primeiro registro para 21/12/16
